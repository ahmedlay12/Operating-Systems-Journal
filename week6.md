...
Baseline system performance metrics were collected remotely via SSH from the workstation.
...
The system remained stable throughout CPU and memory stress testing, with no service interruptions observed. Real-time monitoring confirmed that resource utilisation stayed within acceptable limits, demonstrating that the system is capable of handling moderate workloads. These results provide a useful baseline for future performance comparison and optimisation tasks.
